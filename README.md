...
| | Поля |
|---|---|
| Верх | ⏱ длительность поездки · 🚗 температура салона |
| Центр | **SOC %** (зелёный/жёлтый/красный) · **~N км** запас хода · **расход ↓↑→** |
| Низ | 🔋 температура батареи · ⚡ 12V |

Рамка подсвечивается цветом статуса (SOC или 12V — что хуже).

### Расход и стрелка тренда

**Цифра в центре (кВт·ч/100км)** — средний расход **текущей поездки**: `кВт·ч с начала поездки ÷ км × 100`.

- **`—`** в первые 1 км — мало данных, цифре доверять нельзя (холодный старт, пиковая мощность, разгон с места дают 60–100 кВт·ч/100 — шум)
- **Трип-средний** после 1 км пробега — ваш расход с начала поездки
- **Baseline** (среднее по истории) — когда машина не едет, виджет показывает ambient значение

**Стрелка тренда** включается **после 2 км пробега** и сравнивает расход текущей поездки с вашим обычным стилем:

- **↓ зелёная** — едешь экономнее обычного
- **→ серая** — в пределах обычного
- **↑ жёлтая** — расход выше обычного

Baseline для сравнения — **EMA последних 10 поездок** длиной от 1 км (парковочные манёвры отсекаются). Не привязан к календарной неделе: отражает твой актуальный стиль, а не то, как ты ездил в прошлое воскресенье. Debounce 60 сек — стрелка не дёргается от одной остановки на светофоре.
