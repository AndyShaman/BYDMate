...
Ниже кольца: AI-инсайт, малая карточка здоровья батареи (SoH на Leopard 3 и DiLink 3.0, температура, 12V), два счётчика TRIP 1 и TRIP 2, последние поездки, фильтр периода. Каждый счётчик показывает пробег, расход в кВт·ч (включая стоянку), время в пути и стоимость с момента сброса. Долгое нажатие сбрасывает счётчик, касание открывает детали.
...
- Скажите «в картах»: «поехали домой в картах», «найди заправку в яндекс картах» — маршрут, поиск и показ точки уйдут в Яндекс Карты, без этих слов всё идёт в Навигатор как раньше
- «Найди зарядки рядом»
