...
[О форке](#о-форке-bydmate-dm) | [Возможности](#возможности) | [Скриншоты](#скриншоты) | [Автоматизация](#автоматизация) | [Вывод на приборку](#вывод-на-приборку) | [Голосовой агент](#голосовой-ai-агент) | [AI Инсайты](#ai-инсайты) | [ABRP](#abrp--live-телеметрия) | [Установка](#установка) | [Сборка](#сборка-из-исходников) | [Релизы](https://github.com/hix83/BYDMate-DM/releases) | [Поддержать](SUPPORT.md)
...
Этот репозиторий - рабочий форк [основного BYDMate](https://github.com/AndyShaman/BYDMate), адаптированный под нашу гибридную машину BYD Song L DM-i / DiLink 5.0 и реальные проверки на установленной голове. Цель форка - быстро держать поверх `main` те изменения, которые нужны именно этой комплектации, не ломая upstream и не смешивая экспериментальные доработки с универсальной версией приложения.

Форк существует по трём причинам:

- **Гибридная конфигурация.** В DM-i отличаются ёмкость батареи, профиль машины, расчёт расхода и некоторые источники данных. Форк хранит эти значения и поведение отдельно от универсального BYDMate.
- **Вывод Яндекс.Навигатора на приборку.** На нашей DiLink 5.0 приборка может быть скрыта от обычного `DisplayManager`, но доступна через системный display `2`. Поэтому в форке сохранён DM-путь вывода через `ClusterAnchorActivity`, helper-daemon и временное подавление штатной карты, как в рабочей версии 3.6.
- **Безопасное слияние с upstream.** Мы регулярно подтягиваем свежий `main`, но оставляем проверенные DM-доработки: версия `3.8.1-dm.2` основана на upstream `v3.8.1` и содержит восстановленный рабочий вывод навигатора на приборку.

Что изменено относительно основной ветки:

| Область | Что добавлено в BYDMate-DM |
|---|---|
| **Профиль машины** | DM-i профиль и дефолтная ёмкость батареи для нашей комплектации |
| **Приборка** | Fallback для скрытого display `2`: anchor Activity, nested VirtualDisplay, восстановление штатной проекции при выключении |
| **Навигатор** | Стабильный вывод Яндекс.Навигатора на приборку без потери Surface: helper-daemon не фокусирует nested VirtualDisplay |
| **HUD** | Более осторожная диагностика: наличие SOME/IP gateway само по себе не считается подтверждением HUD |
| **Релизы** | Отдельные DM-сборки и теги вида `v3.8.1-dm.2` в [релизах форка](https://github.com/hix83/BYDMate-DM/releases) |

Этот форк не заменяет основной BYDMate. Всё общее и полезное лучше возвращать upstream, а здесь держать только то, что зависит от нашей машины, нашей прошивки DiLink и проверенного сценария установки.
...
| **Voice** | Голосовой агент | Русский офлайн-ассистент: распознавание речи на устройстве, AI-агент (~25 инструментов), нейросетевая озвучка |
...
*Батарея, тарифы, валюта, источник инсайтов (локальный / OpenRouter), экспорт данных*
...
| **9 видов действий** | Команда машины, тихое/звуковое уведомление, запуск приложения, звонок, навигация, URL, Яндекс.Музыка, Охранный режим |
...
<img src="docs/screenshots/cluster-projection-icon.jpg" alt="Иконка проекции на приборку на главном экране" width="800">

**2.** На появившейся карте нажмите кнопку **IPC** в нижней панели, пока приборка не перейдёт в полноэкранный режим.

<img src="docs/screenshots/cluster-projection-ipc-full.jpg" alt="Кнопка IPC переводит приборку в полноэкранный режим" width="800">
...
Полноценный русский голосовой помощник внутри BYDMate. Речь распознаётся прямо на головном устройстве, без интернета: нейросеть GigaAM v2 слушает и понимает по-русски. Простые команды исполняются мгновенно локальным разбором, всё остальное решает AI-агент: языковая модель с 25 инструментами, которая сама читает данные машины, управляет функциями кузова, строит маршруты, запускает сценарии и отвечает голосом.
...
1. **Настройки → Голос → Распознавание речи**: скачайте модель распознавания (226 МБ, один раз, хранится локально).
...
У агента 25 инструментов. Примеры фраз:

**Машина и климат**
- «Сколько заряда?», «Какая температура за бортом?», «Какой запас хода?» (живое состояние машины)
- «Открой окна», «Включи подогрев сиденья», «Поставь климат на 22» (полный каталог из 77 команд, см. ниже)
...
### Управление машиной: каталог из 77 команд
...
- Голосовой журнал (последние 50 распознаваний с результатами, для самодиагностики) живёт только в оперативной памяти и не сохраняется на диск. Экран журнала: Настройки → Голос → «Голосовой журнал», там видно, как фраза была распознана, куда ушла и чем закончилась.
- Для проверки агента без микрофона есть текстовый чат (отладочный экран в настройках).

### Отключение штатного помощника BYD

Внизу раздела «Голос» есть тумблер, который отключает штатный голосовой помощник BYD, чтобы он не просыпался параллельно с агентом. Полностью обратимо, применяется после перезагрузки машины (и включение, и выключение). По умолчанию выключено.
...
Если в вашей машине SoH доступен и вы хотите помочь добавить поддержку, заведите [Issue](https://github.com/hix83/BYDMate-DM/issues) с указанием модели и года выпуска.
...
Если что-то не работает или показывает странное, заведите [Issue](https://github.com/hix83/BYDMate-DM/issues) с указанием модели машины и версии прошивки DiLink. Нам нужны такие отчёты, чтобы расширять поддержку.
...
1. Скачайте BYDMate-DM APK из [**Releases**](https://github.com/hix83/BYDMate-DM/releases)
...
Логика локального движка — `LocalInsightEngine.kt`: детерминированные пороги + шаблоны строк (`local_insight_*` в `strings.xml`). Тексты локализованы (ru / en / zh).
...
git clone https://github.com/hix83/BYDMate-DM.git
cd BYDMate-DM
