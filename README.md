...
**Расход и температура.** На вкладке «Поездки» чип «Температура» открывает график: точка на каждую поездку, по горизонтали температура за бортом, по вертикали расход. Медианы по холоду, межсезонью и теплу и одна фраза-вывод, на сколько процентов зима дороже лета. Нужны 8 поездок с известной температурой, приложение запоминает её с этой версии.

**Маршрут на разделённом экране и «Поехали» за вас.** Скажите «поехали на работу»: ассистент сам свернёт разделение, построит маршрут в Яндекс Навигаторе, нажмёт «Поехали» и вернёт панели на место. На «построй маршрут домой» он только построит маршрут, кнопку нажмёте сами.
...
| **Bat** | Здоровье батареи и «Техника» | Карточка: SoH (на Leopard 3), температура, 12V, разброс ячеек, изоляция. Тап открывает экран «Техника» с живыми параметрами батареи, моторов, климата и шин, включая долю мощности перед/зад в процентах |
...
| **CSV** | Экспорт данных | Поездки, зарядки, тарифные периоды, GPS-треки и разряд на стоянке в CSV, со стоимостью, тарифом и потерями |
...
| **18 видов действий** | Команда автомобилю, уведомление, запуск приложения, звонок, маршрут в Я.Навигаторе, открыть URL, Яндекс.Музыка, запуск ролика на YouTube, пауза, громкость медиа, охранный режим, точка доступа Wi-Fi, озвучить текст, запрос агенту, главный экран BYDMate, вывод на приборку, разделение экрана (запуск, закрытие, переключение), «Переключить» (кузов, климат, сиденья) |
...
**Действие «Переключить»** одной кнопкой открывает и закрывает: багажник, передний багажник, люк, замки, вывод на приборку, аварийку, климат, подогрев и обдув сидений водителя и пассажира. Состояние берётся из машины. Сиденье, которое сейчас работает, выключится, а выключенное вернётся на ту ступень, которую вы задавали последней (по умолчанию третью).
...
На части прошивок (DiLink 4.0, Song Plus 2021 и похожие) экран приборки скрыт от приложений, и «Заводской» режим там работать не может: приложение само находит приборку через служебный канал и выводит навигатор «Расширенным» режимом, независимо от выбранного. Поддержка этих прошивок проверяется.
...
- «Открой окна», «Включи подогрев сиденья», «Поставь климат на 22» (полный каталог из 95 команд, см. ниже)
...
### Управление машиной: каталог из 95 команд
...
Это сделано специально: вы видели чек и счётчик, приложение нет. Такие записи остаются как есть при любой смене тарифов, а в экспорте у них стоит `cost_manual = true`.
...
**Настройки → Сервис и данные → «Экспорт CSV»** кладёт в папку Download пять файлов с меткой времени. Стоимость есть в трёх из них:

| Файл | Что внутри |
|------|-----------|
| `bydmate_trips_*.csv` | Поездки. Колонка `cost` — стоимость, `tariff_rate` — цена киловатт-часа, по которой она посчитана |
| `bydmate_charges_*.csv` | Зарядки. `cost` — стоимость, `meter_kwh` — ваше показание счётчика, `cost_manual` — стояла ли цена руками, `tariff_rate` — цена киловатт-часа, `loss_pct` — учтённые потери |
| `bydmate_tariff_periods_*.csv` | Все ваши периоды: дата начала, цены дома и DC, потери AC и DC, правило расчёта поездок, валюта |

Остальные два файла — точки GPS-треков и разряд на стоянке. Валюта в приложении одна на все периоды, меняется в том же разделе настроек.
...
- При первом включении **на каждой новой сети** на экране появится системный диалог «Allow wireless debugging on this network?»: отметьте «Always allow on this network» и нажмите ALLOW. Поэтому удобнее всегда использовать одну и ту же сеть.
...
- **«Голосовые команды понимают только русскую речь»** — переключите язык приложения: Настройки → Приложение → Язык: Русский.
